...
export module cdkDiffWorkflow {
      ...
      diff: {
        ...
        steps: [
          ...
          },
          {
            ...
            with: {
              'role-to-assume': 'QA_OIDC_ROLE_ARN',
  ...
  function replaceOidcRoleName(options: CDKDiffOptionsConfig, workflow: any) {
    const steps = workflow.jobs?.diff?.steps;

    if (!steps) {
      throw new Error('Could not find the diff job in the workflow');
    }

    const configureAwsCredentialSteps = steps.filter((s: any) => !!s?.with?.['role-to-assume']);
    for (const step of configureAwsCredentialSteps) {
      step.with['role-to-assume'] = step.with['role-to-assume'].replace('QA_OIDC_ROLE_ARN', options.oidcQaRoleArn);
      step.with['role-to-assume'] = step.with['role-to-assume'].replace(
        'STAGING_OIDC_ROLE_ARN',
        options.oidcStagingRoleArn,
      );
      step.with['role-to-assume'] = step.with['role-to-assume'].replace('PROD_OIDC_ROLE_ARN', options.oidcProdRoleArn);
    }
  ...
  export function addCdkDiffWorkflowYml(
    ...
    new YamlFile(project, '.github/workflows/cdk-diff.yml', {
      obj: { ...replaceOidcRoleName(options, defaultWorkflow), ...override },
    });
  }
}
